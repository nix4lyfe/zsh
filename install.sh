...
if [[ "$prompt" == "y" || "$prompt" == "Y" || "$prompt" == "yes" || "$prompt" == "Yes" || "$prompt" == "YES" ]];
then
rm scripts/laptopmode.sh
sed -i'' -e '/alias laptopmode="$HOME\/scripts\/laptopmode.sh"/d' scripts/aliases
fi
cp -R ./{scripts,.zshrc,.zhistory,.config,.local,.vimrc} $HOME/
cd $HOME/
chsh -s /bin/zsh
...
printf "%s " "Would you like to delete the downloaded zsh folder? <y/N>"
read prompt
if [[ "$prompt" == "y" || "$prompt" == "Y" || "$prompt" == "yes" || "$prompt" == "Yes" || "$prompt" == "YES" ]];
then
rm -rf zsh
